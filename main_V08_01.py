	...
	def plotDF(self,event=None):
		...
		if self.inputTab:
			...
			if self.topPlot_YVales_combo.get() != '' and self.sharedX_combo.get() != '' and self.bottomPlot_YVales_combo.get():
				...
				# self.ax1.plot(self.inputTab.df[x],self.inputTab.df[yTop])
				# self.ax2.plot(self.inputTab.df[x],self.inputTab.df[yBot])
				...
				# self.plot = FigureCanvasTkAgg(self.fig, self.figureFrame)
				# self.plot.get_tk_widget().pack(side='top', fill='both', expand=True)

				# ax1.plot(self.inputTab.df[x],self.inputTab.df[yTop])
				# ax2.plot(self.inputTab.df[x],self.inputTab.df[yBot])
				# self.plot.get_tk_widget().pack(side='top', fill='both', expand=True)
				# self.plot1.set_xdata(self.inputTab.df[x])
				# self.plot1.set_ydata(self.inputTab.df[yTop])
				'''
...
class MainPlotWindow():
	...
	def updatePlot(self,df,x,yTop,yBot):
		...
		# change to Datetime if Date is column header...
		# this is temporary
		if x == 'Date':
			xVals = pd.to_datetime(df[x].values)
		else:
			xVals = df[x].values
