...
# inherit from tk.Tk class
	def __init__(self, master, *args, **kwargs):
		...
		### the main Tk Window
		master.title(" Dataframe plotting - V2020-08.02.01")
...
##		## canvas is the only scrollable container in tkinter
		## need to place the Canvas into the self frame, then a frame into that canvas
...
##		## set the notebook into the main frame/window of the UI
		self.notebook.pack(anchor='nw', fill='both', expand=True)#grid(row=0,column=0,pady=10)
...
##		## canvas is the only scrollable container in tkinter
		## need to place the Canvas into the self frame, then a frame into that canvas
		self.canvas = tk.Canvas(self, borderwidth=0, background="white")
		## tack on a new frame, placed within the canvas, now we can scroll the info in the frame..
		self.frame = tk.Frame(self.canvas, bg="white")
		self.frame.pack(anchor='nw', fill='both', expand=True)

		## set Vertical ScrollBar (vsb) and configure it to y scroll command onto the canvas
		self.vSB = tk.Scrollbar(self, orient="vertical", command=self.canvas.yview)

		# self.vSB.pack(side="right", fill="y")
		# self.canvas.pack(side="left", fill="both", expand=True)		

		self.canvas.grid(row=0, column=0, sticky='nwes')#side="left", fill="both", expand=True)
		self.vSB.grid(row=0, column=10, sticky="ns")#, fill="y")
		...
		self.frame.bind("<Configure>", lambda event: self.canvas.configure(scrollregion=self.canvas.bbox("all")))#frameConfiguration)
...
##		## Main input file area... refer to all the objects/functions to self, theyre just in the parent object/frame
		self.inputFile_label = tk.Label(self.frame, text="Input File:", font=("Helvetica",10), bg="white")
		...
		self.inputFile_combo = ttk.Combobox(self.frame, width=50, values=glob.glob("./*") )
		...
		self.inputFile_browseButton = tk.Button(self.frame, text="Browse for File...", command=self.browseFiles)
		...
		self.inputFile_clearButton = tk.Button(self.frame, text="Clear File", width=25, command=self.clearFilename)
...
##		## set the input file type - this will be of types "we" know, so csv, specific data files, etc
		self.fileType_label = tk.Label(self.frame, text="File Type:", font=("Helvetica",10), bg="white")
		...
		self.fileType_combo = ttk.Combobox(self.frame, width=25, values=['csv','other types to come...'])
...
##		## button to actually read in the file and do the processing
		self.processFile_button = tk.Button(self.frame, text="Process File", font=("Helvetica",14), width=40, height=1, command=self.processFile)
...
##		## add in a text frame to show the log, ie initial column types and then converted
		...
		self.processedFile_log = Text(self.frame, width=60, height=30, font=("Courier",8), borderwidth=2, padx=2, pady=2)
...
##		## add in a text frame to print some info on data file?
		self.processedFile_info = Text(self.frame, width=80, height=30, font=("Courier",8), borderwidth=2, padx=2)
...
# so the parent is the object that is calling this window and thus we will set the widgets in that object,
	...
	def __init__(self, master, *args, **kwargs):
		...
		self.columnconfigure(0, weight=1) # need these to have rows and column have non-zero weight
		self.rowconfigure(0, weight=1)
...
##		## canvas is the only scrollable container in tkinter
		...
		self.canvas.grid(row=0, column=0, sticky='nwes')#side="left", fill="both", expand=True)
		self.vSB.grid(row=0, column=15, sticky="ns")#, fill="y")
...
##		## Main input file area... refer to all the objects/functions to self, theyre just in the parent object/frame
		self.inputDF_label = tk.Label(self.frame, text="Input DataFrame", font=("Helvetica",10), bg="white")
		self.inputDF_label.grid(column=1,row=2,padx=5,pady=5,sticky="WNES") # sticky helps push away from borders here

		## input file combo box - default values are the files in current directory
		self.inputDF_entry = tk.Entry(self.frame, width=50, borderwidth=3)
		self.inputDF_entry.insert(END, 'NULL')
		self.inputDF_entry.grid(column=2,row=2,padx=5,pady=5,sticky="WNES")

		## browse buttong to search for files outside current directory - calls browseFiles functions
		self.inputDF_importButton = tk.Button(self.frame, text="Import DF", bg='yellow', command=self.importDF)
		...
		self.inputDF_plotButton = tk.Button(self.frame, text="Plot DF", bg='red',command=self.plotDF)
...
##		## figure axes info, will be left side
		self.figureAxes = tk.Frame(self.frame, borderwidth=2, width=100, height=frameHeight, bg='#ADD8E6')
		self.figureAxes.grid(column=1, row=3)
##		## figure frame, will host the figure
		self.figureFrame = tk.Frame(self.frame,bg='gray', width=1600, height=frameHeight+150)
		self.figureFrame.grid(column=2, row=3,columnspan=5,)
##		## figure actions, buttons etc, will be on right side 
		self.figureActions = tk.Frame(self.frame, borderwidth=2, width=100, height=frameHeight, bg='#3EA99F')
...
##		# button to refresh
		...
		self.bottomPlot_YVales_label.grid(row=8, column=0, padx=10)
...
if __name__ == "__main__":
	...
	main_window.pack(anchor='nw', fill='both', expand=True)
